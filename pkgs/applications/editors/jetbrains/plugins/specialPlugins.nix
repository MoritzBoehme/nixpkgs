{ lib, delve, autoPatchelfHook, stdenv, patchelf, glibc, gcc-unwrapped, nodejs }:
...
# debugger, but that needs various linking fixes. The changes here replace it with the system one.
{
  ...
  "17718" = {
    buildInputs = [ patchelf glibc gcc-unwrapped nodejs ];
    commands =
      let
        libPath = lib.makeLibraryPath [ glibc gcc-unwrapped ];
      in
      ''
